...
object Ej1 extends App {
  ...
  NOTA: La persona tiene estrictamente prohibido realizar la correspondencia a través de sentencias match, case o if puesto que estaría violando su contrato. No hagas que pierda su empleo :(

  Ejemplo de entrada y salida:

  encryptVowel(List('a', 'i', 'e', 'a')) => List(L, $, 9, L) encryptVowel(List('a')) => List(L) encryptVowel(List('i', 'e')) => List($, 9)
   * */
